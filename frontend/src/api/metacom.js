...
class Metacom extends EventEmitter {
  ...
  async message(data) {
    ...
    if (type === "callback") {
      ...
      const { name: streamName, size, status } = packet;
      const stream = this.streams.get(id);
      if (streamName && typeof streamName === "string" && Number.isSafeInteger(size)) {
        if (stream) {
          console.error(new Error(`Stream ${streamName} is already initialized`));
        } else {
          const nextStream = new MetaReadable(id, streamName, size);
          this.streams.set(id, nextStream);
        }
      } else if (!stream) {
